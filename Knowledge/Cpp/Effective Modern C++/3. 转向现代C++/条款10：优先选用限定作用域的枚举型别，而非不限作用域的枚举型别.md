...
虽然这样的写法可以获取到正确的数据信息，但是作为程序员我们很难记得数字 0,1,2,.... 与 名字 电子邮件 声望值 .... 之前的一一对应关系，并且这样的代码也很容易让后续的维护者摸不着头脑。此时我们就希望对存储的信息使用枚举的方式进行标记，好在不限定作用域的枚举类型是可以进行隐式类型转换的，因此此时使用不限定作用域的枚举类型恰好可以优雅的解决此问题：

``` C++

enum UserInfoFields {eName, eEmail, eReputation};

UserInfo uInfo;
...

auto value = std::get<eEmail>(uInfo); // 利用不限定作用域枚举
                // 可以进行隐式类型转化的特性，此处的eEmail会
                // 被隐式转化为std::size_t类型
                
```

而如果此时我们仍然使用限定作用域枚举类型，那么实现该功能的代码版本就要麻烦的多了：

``` C++

enum class UserInfoFields {eName, eEmail, eReputation};

UserInfo uInfo;
...

auto value = std==get<static_cast<std==size_t>(UserInfoFields::eEmail)>(uInfo);  // 手动调用显示类型转化
                                // 将枚举转化为std::size_t

```

也许你会说：“其实也不用那么麻烦，只要封装一个函数将枚举转化成std::size_t就可以了”，类似这样：

``` C++

enum class UserInfoFields {eName, eEmail, eReputation};

size_t EnumToSizeT(UserInfoFields e)
{
...
auto value = std==get<EnumToSizeT(UserInfoFields==eEmail)>(uInfo);  // 有问题

```

实际上这样是不可行的，这是因为`std::get<>()`是个模板函数，其尖括号内的的参数必须在编译器就计算出结果，这就意味着必须使用`constexpr`函数(详见：[[条款15：只要有可能使用constexpr就使用它|条款15]])。

如果我们想做的更通用一点，那么仅使用`constexpr`函数还不够，因为更通用的做法是要配合和支持任意枚举类型，因此就需要使用`constexpr`模板函数。较之于返回`std==size_t`，我们更应该返回枚举的底层类型。这可以通过`std==underlying_type`这个_type trait_获得。（参见[[条款9：优先选用别名声明而非typedef|条款9]])关于_type trait_的内容）。最终我们还要再加上`noexcept`修饰（参见[[条款14：只要函数与不会抛出异常就为其加上noexcept声明|条款14]]），因为我们知道它肯定不会产生异常。根据上述分析最终得到的`toUType`函数模板在编译期接受任意枚举名并返回它的值：
...
在C++14中，`toUType`还可以进一步用`std==underlying_type_t`（参见[[条款9：优先选用别名声明而非typedef|条款9]])代替`typename std==underlying_type<E>::type`打磨：
...
还可以再用C++14 `auto`（参见[[条款3：理解decltype|条款3]]）打磨一下代码：
...
这仍然比使用非限域`enum`要写更多的代码，但同时它也避免命名空间污染，防止不经意间使用隐式转换。大多数情况下，你应该会觉得多敲几个（几行）代码作为避免使用未限域枚举带来的弊端的代价是值得的。
