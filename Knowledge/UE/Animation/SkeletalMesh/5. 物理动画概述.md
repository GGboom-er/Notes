...
![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_001.gif]]

左侧为纯序列帧对象，可以明显看到动作播放时会和环境(墙壁)发生穿插，而右侧启用了物理的动作则可以和墙壁碰撞发生互动。又或者由于序列帧动画不会与环境产生交互从而导致动画结果呆板不真实，如下图所示：

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_002.gif]]
...
![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_003.png]]
...
![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_004.png]]
...
![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_005.png]]

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_006.png]]
...
![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_007.png]]

- 拖动调整约束位置朝向时合理使用Alt+旋转/拖拽 Shift+Alt+旋转/拖拽，直接调整朝向和位置可能会同时修改约束在父空间和子空间的位置朝向，不利于调整成理想的效果

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_008.png]]

- 查看单个物理约束效果时，可以先将其他物理体设置为运动学并关闭重力，进开启当前物理体(断开子物体约束或者同时开启子物体物理模拟)。单个物理体效果调整完成后再调整体效果。

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_009.png]]

- 如果需要删除现有约束重新添加或者增加新的约束，一定要确定父子对象关系：父物理体约束子物理体。(父子级会影响约束结果)

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_010.png]]

- 调整物理体类型(eg: 把胶囊体换成Box)

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_011.png]]

- 约束细节对整体效果的影响，当每个物理体的约束已经调整到合理范围后就需要对整体效果进行微调(Ragdoll受重力影响向前倒地/向后到底 腿部分开/聚拢等)

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_012.png]]

## 3.3 物理动画效果展示

参考官方提供示例，物理动画可以用来与场景交互(解决穿插问题)，模拟局部受伤/受攻效果，模拟驾驶时颠簸，模拟攀爬等悬空时身体受外力产生的局部摆荡等效果，如下所示，更多效果请参见官方示例。

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_013.gif]]

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_014.gif]]

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_015.gif]]

![[Knowledge/UE/Animation/SkeletalMesh/attachments/Pic_016.gif]]
